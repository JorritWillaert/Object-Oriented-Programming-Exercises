...
class MatrixTestImmutable {
	...
	void test() {
		...
		double[] data1ScaledWithOnePointFive = {1.5, 2.25, 3., 3.75, 7.5, 7.845, 6., 9., 1.5, 3.45, 6.75, 9.3};
		double[] dataSum = {3., 4, 5., 6., 6.2, 11., 7., 12.2, 1.5, 4.0, 7.2, 10.9};
		...
		assertEquals(2.3, matrix1.getElement(3, 2));
		data1[10] = 5.2;
		assertEquals(2.3, matrix1.getElement(3, 2)); //Test for representation object
		...
		assertArrayEquals(doubleMatrixRowMajorScaled, dataSum);
		...
		double[] dataSumColumnMajor = {3., 6.2, 1.5, 4., 11., 4.0, 5., 7., 7.2, 6., 12.2, 10.9};
		...
		for (int i = 0; i < dataSumArrayOfRows.length; i++) {
			assertArrayEquals(doubleMatrixArrayOfRowsSum[i], dataSumArrayOfRows[i]);
		}
	}
}
